...
{
	public function cargasDescargas( $idSupervisor, $idRepartidor,
		$dniSupervisor, $dniRepartidor, $fechaCarga, $cantidadCargas,
		array
		 $registrosArticulos ) {
...
//echo "<br>";
		...
		for ( $i = 0; $i < $cantidadCargas; $i++ ) {
			...
			$RegistroIdCarga = $insertar->insertarCarga(
				$fechaCarga, $plata, $idSupervisor,
				$dniSupervisor, $idRepartidor,
				$dniRepartidor );
			echo "<br>";
...
//print_r($RegistroIdCarga);
			...
			for ( $x = 0; $x < $iteracionDetalle; $x++ ) {
				...
				$insertar->InsertarDetalleCarga(
					$CantidadDetalle, $idCarga, $idArticulo
				);
			}
