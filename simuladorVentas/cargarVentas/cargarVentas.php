...
// indicas las cantidad de ventas ha generar
		...
		for ( $i = 0; $i < $cantidadVentas; $i++ ) {
			...
			// Existen 6 posibles casos de inserción en la tabla de ventas. Se elige uno al azar
			{
				case 1:
					...
					$filaCliente = $azar->alAzar( 0, count(
						$registrosClientes ) - 1 );

					$idCliente = $registrosClientes[
						$filaCliente][

					 "ClientesDirectos_Persona_IdCliente"];
					$dniCliente = $registrosClientes[
						$filaCliente][

					"ClientesDirectos_Persona_DNICliente"];

					$filaRepartidor = $azar->alAzar( 0,
						count( $registrosRepartidores )
						 - 1 );

					$idRepartidor = $registrosRepartidores
						[$filaRepartidor][
						"Persona_IdRepartidor"];
					$dniRepartidor = $registrosRepartidores
						[$filaRepartidor][
						"Persona_DNIRepartidor"];

					$RegistroIdVenta = $insertar->
						insertarVenta( $cantidadEnvases
						, $fechaVenta, $pago,
						$idCliente, $dniCliente,
						$idDistribuidor,
						$dniDistribuidor, $idRepartidor
						, $dniRepartidor, $comprobado,
						$total, $caso );
					echo "<br>";
					echo

				     "insertado en  ventas. CLIENTE/REPARTIDOR";
					echo "<br>";
					break;
				case 2:
					...
					$filaDistribuidor = $azar->alAzar( 0,
						count( $registrosDistribuidores
						) - 1 );

					$idDistribuidor =
						$registrosDistribuidores[
						$filaDistribuidor][
		      ...
		      "insertado en  ventas. CLIENTE/DISTRIBUIDOR NO PERMITIDO";
					echo "<br>";
	     ...
	     "insertado en  ventas. CLIENTE/REPARTIDOR/REPARTIDOR NO PERMITIDO";
				...
				"insertado en  ventas. REPARTIDOR NO PERMITIDO";
					echo "<br>";
				...
				"insertado en  ventas. DISTRIBUIDOR/REPARTIDOR";
					echo "<br>";
				...
				case 7:
					...
					echo
					   ...
					   "insertado en  ventas. DISTRIBUIDOR";
					echo "<br>";
...
//print_r($registrosArticulos);
			...
			for ( $x = 0; $x < $iteracionDetalle; $x++ ) {
				...
				$indice = $azar->alAzar( 0, count(
					$registrosArticulos ) - 1 );
				$idArticulo = $registrosArticulos[$indice]
					["IdArticulo"];
				$CantidadDetalle = $azar->alAzar( 1, 10 );
				$subtotal        = $CantidadDetalle *
					$registrosArticulos[$indice]["Precio"];
				$total = $total + $subtotal;
				$insertar->InsertarDetalleVenta(
					$CantidadDetalle, $subtotal, $idVenta,
					$idArticulo );
