...
'SubTotal' => $subtotal, 'Venta_IdVentas' => $idVenta, 'Articulo_IdArticulo' =>
							       $idArticulo ) );
	...
	public function insertarVenta( $envasesVacios, $fechaVenta, $pago,
     $idCliente, $dniCliente, $idDistribuidor, $dniDistribuidor, $idRepartidor,
				   $dniRepartidor, $comprobado, $total, $caso )
	{
		$db = new Database();
		$db->connect();
		...
		switch ( $caso ) {
			case '1':
				$db->insert( 'ventas', array( 'EnvasesVacios'
		    => $envasesVacios, 'Fecha' => $fechaVenta, 'Pago' => $pago,
			      'ClienteDirecto_Persona_IdCliente' => $idCliente,
