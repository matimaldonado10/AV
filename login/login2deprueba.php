...
function codificarCaracteresAUtf8($RegistroClientesRepartidor)
{
	...
	for ($i = 0; $i < count($RegistroClientesRepartidor); $i++) {
		...
		$RegistroClientesRepartidor[$i][
"ClientesDirectos_Persona_IdCliente"]  = utf8_encode($RegistroClientesRepartidor
				  [$i]["ClientesDirectos_Persona_IdCliente"]);
		$RegistroClientesRepartidor[$i][
"ClientesDirectos_Persona_DNICliente"] = utf8_encode($RegistroClientesRepartidor
				  [$i]["ClientesDirectos_Persona_DNICliente"]);
		//$RegistroClientesRepartidor[$i]["Apellido"] = utf8_encode($RegistroClientesRepartidor[$i]["Apellido"]);
		$RegistroClientesRepartidor[$i]["Apellido"] = iconv("CP1255",
			"UTF-8", $RegistroClientesRepartidor[$i]["Apellido"]);

		//$RegistroClientesRepartidor[$i]["Nombre"] = utf8_encode($RegistroClientesRepartidor[$i]["Nombre"]);

		$RegistroClientesRepartidor[$i]["Nombre"] = iconv("CP1255",
			   "UTF-8", $RegistroClientesRepartidor[$i]["Nombre"]);

		$RegistroClientesRepartidor[$i]["Telefono"]   = utf8_encode(
				  $RegistroClientesRepartidor[$i]["Telefono"]);
		$RegistroClientesRepartidor[$i]["Email"]      = utf8_encode(
				     $RegistroClientesRepartidor[$i]["Email"]);
		$RegistroClientesRepartidor[$i]["Direccion"]  = utf8_encode(
				 $RegistroClientesRepartidor[$i]["Direccion"]);
		$RegistroClientesRepartidor[$i]["Referencia"] = utf8_encode(
				$RegistroClientesRepartidor[$i]["Referencia"]);
		$RegistroClientesRepartidor[$i]["Barrio"]     = utf8_encode(
				    $RegistroClientesRepartidor[$i]["Barrio"]);
...
if ($login) {
	...
	$response->exito = true;
	if ($usr == "sup") {
		$data->id           = $RegistroSupervisor[0][
						       "Persona_IdSupervisor"];
		$dni                = $RegistroSupervisor[0][
		...
		if (strcmp($primerLogin, "true") == 0) //IGUAL A CERO SIGNIFICA QUE $primerLogin es igual a true, por lo tanto, el usuario se loguea por primera vez
		{
			...
			$RegistroClientes = $tablas->
					     obtenerClientesDeRepartidor($dni);
			...
			for ($i = 0; $i < count($RegistroClientes); $i++) {
				...
				// code...
				if ($i == 0) {
					$dniDistinto = $RegistroClientes[$i][
									"DNI"];
					$indice      = $i;
				}

				if (strcmp($dniDistinto, $RegistroClientes[$i][
								 "DNI"]) == 0) {
					$aux = array('Dia' => $RegistroClientes
								  [$i]['Dia']);
					array_push($Dias, $aux);

				} else {

					$RegistroClientesRepartidor[$j][
"ClientesDirectos_Persona_IdCliente"]  = $RegistroClientes[$indice]["IdPersona"]
									      ;
					$RegistroClientesRepartidor[$j][
    "ClientesDirectos_Persona_DNICliente"] = $RegistroClientes[$indice]["DNI"];
					$RegistroClientesRepartidor[$j][
"Apellido"]                            = $RegistroClientes[$indice]["Apellido"];
					$RegistroClientesRepartidor[$j][
 "Nombre"]                              = $RegistroClientes[$indice]["Nombre"];
					$RegistroClientesRepartidor[$j][
"Telefono"]                            = $RegistroClientes[$indice]["Telefono"];
					$RegistroClientesRepartidor[$j]["Email"
	 ]                               = $RegistroClientes[$indice]["Email"];
					$RegistroClientesRepartidor[$j][
"Direccion"]                           = $RegistroClientes[$indice]["Direccion"]
									      ;
					$RegistroClientesRepartidor[$j][
"Referencia"]                          = $RegistroClientes[$indice]["Referencia"
									     ];
					$RegistroClientesRepartidor[$j][
 "Barrio"]                              = $RegistroClientes[$indice]["Barrio"];
					$RegistroClientesRepartidor[$j]["Dia"]
								       = $Dias;
					$j++;
			...
			$registroClientesFinal = codificarCaracteresAUtf8(
						  $RegistroClientesRepartidor);
